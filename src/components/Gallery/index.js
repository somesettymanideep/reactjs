...
class App extends Component {
  state = {activeThumbnailIb: imagesList[0].id}

  setActiveThumbnailId = id => {
    this.setState({
      activeThumbnailIb: id,
    })
  }

  render() {
    const {activeThumbnailIb} = this.state
    const {imageUrl, imageAltText} = imagesList[activeThumbnailIb]
          ...
          <ul className="icon-container">
            {imagesList.map(eachImage => (
              <ThumbnailItem
                ...
                isActive={activeThumbnailIb === eachImage.id}
